...
function generate(type, title, docs, filename, resolveLinks) {
    ...
    var docData = {
        ...
        docs: docs.map(function(doc) {
            if (doc.kind === 'typedef') {
                doc.resultTypedef = isResultTypedef(doc);
            }
            return doc;
        })
    };

    if (type === 'Interface' && docs[0] && docs[0].memberof) {
        docData.title = docs[0].memberof + '.' + title;
    }
        ...
        items.forEach(function(item) {
            ...
            if ( !hasOwnProp.call(item, 'longname') ) {
                ...
                if (conf.templates.default.useLongnameInNav) {
                    ...
                    if (itemHeading === 'Interfaces' && item.memberof) {
                        displayName = item.memberof + '.' + item.name;
                    } else {
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    view.sortTypedefs = sortTypedefs;
...
function isResultTypedef(doclet) {
    return doclet.kind === 'typedef' && doclet.name.endsWith('Result');
}

// Après la fonction isResultTypedef(), ajoutez cette fonction pour trier les typedefs
function sortTypedefs(typedefs) {
    if (!typedefs || !typedefs.length) return typedefs;
    
    return typedefs.sort((a, b) => {
        const aIsResult = isResultTypedef(a);
        const bIsResult = isResultTypedef(b);
        
        if (aIsResult && !bIsResult) return -1;
        if (!aIsResult && bIsResult) return 1;
        return 0;
    });
}
